...
class WaterTreatmentLogMainTileView extends StatefulWidget {
  const WaterTreatmentLogMainTileView({super.key});

  @override
  State<WaterTreatmentLogMainTileView> createState() =>
      _WaterTreatmentLogMainTileViewState();
}

class _WaterTreatmentLogMainTileViewState
    extends State<WaterTreatmentLogMainTileView> {
  ...
  Future<bool> loadRecords() async {
    try {
      recordList = await WaterTreatmentLog().fetchAllRecords();
      debugPrint(recordList!.isEmpty
          ? "Empty List"
          : "${recordList?[0].waterTreatmentLogId}");
      return true;
    } catch (e) {
  ...
  Future<bool> deleteRecord(WaterTreatmentLogModel dataObject) async {
    try {
      int recordId = await WaterTreatmentLog().deleteRecord(model: dataObject);
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        appBar:
            SimpleFormsAppBar(title: "Water Treatment Log", isMainTile: true),
        //
        ...
        floatingActionButton: Padding(
          ...
          child: SB(
            ...
            child: FloatingActionButton(
              onPressed: () async {
                dynamic result =
                    await Get.toNamed(NavRoute.waterTreatmentLogAdd);
                if (result == true) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        child: SimpleFormsTileViewCard(
                          ...
                          onDelete: onDelete,
                          viewRouter: NavRoute.waterTreatmentLogView,
                          updateRouter: NavRoute.waterTreatmentLogUpdate,
                          ...
                          textFieldThreeLabel: "Description",
                          textFieldThreeData:
