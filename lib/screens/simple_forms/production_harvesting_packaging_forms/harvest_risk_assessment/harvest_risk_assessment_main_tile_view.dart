...
class HarvestRiskAssessmentMainTileView extends StatefulWidget {
  const HarvestRiskAssessmentMainTileView({super.key});
  @override
  State<HarvestRiskAssessmentMainTileView> createState() =>
      _HarvestRiskAssessmentMainTileViewState();
}

class _HarvestRiskAssessmentMainTileViewState
    extends State<HarvestRiskAssessmentMainTileView> {
  final StateController stateController = Get.find();
  late Future<bool> loadingFuture;
  List<HarvestRiskAssessmentModel>? recordList;
  ...
  Future<bool> loadRecords() async {
    try {
      recordList = await HarvestRiskAssessment().fetchAllRecords();
      debugPrint(recordList!.isEmpty
          ? "Empty List"
          : "${recordList?[0].harvestRiskAssessmentId}");
      return true;
    } catch (e) {
  ...
  Future<bool> deleteRecord(HarvestRiskAssessmentModel dataObject) async {
    try {
      int recordId =
          await HarvestRiskAssessment().deleteRecord(model: dataObject);
        ...
        floatingActionButton: Padding(
          ...
          child: SB(
            ...
            child: FloatingActionButton(
              onPressed: () async {
                dynamic result =
                    await Get.toNamed(NavRoute.harvestRiskAssessmentAdd);
                if (result == true) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        child: SimpleFormsTileViewCard(
                          ...
                          onDelete: onDelete,
                          viewRouter: NavRoute.harvestRiskAssessmentView,
                          updateRouter: NavRoute.harvestRiskAssessmentUpdate,
                          ...
                          textFieldFourLabel: "Harvest Date",
                          textFieldFourData:
                              recordList?[index].harvestDate ?? "",
                        ),
                        onTap: () {
                          //open view
