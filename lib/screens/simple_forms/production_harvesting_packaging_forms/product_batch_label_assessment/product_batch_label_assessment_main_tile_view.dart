...
class ProductBatchLabelAssessmentMainTileView extends StatefulWidget {
  const ProductBatchLabelAssessmentMainTileView({super.key});

  @override
  State<ProductBatchLabelAssessmentMainTileView> createState() =>
      _ProductBatchLabelAssessmentMainTileViewState();
}

class _ProductBatchLabelAssessmentMainTileViewState
    extends State<ProductBatchLabelAssessmentMainTileView> {
  ...
  Future<bool> loadRecords() async {
    try {
      recordList = await ProductionBatchLabelAssessment().fetchAllRecords();
      debugPrint(recordList!.isEmpty
          ? "Empty List"
          : "${recordList?[0].productionBatchLabelAssessmentId}");
      return true;
    } catch (e) {
  ...
  Future<bool> deleteRecord(
      ProductionBatchLabelAssessmentModel dataObject) async {
    try {
      int recordId = await ProductionBatchLabelAssessment()
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: SimpleFormsAppBar(
            title: "Production Batch Label Assessments", isMainTile: true),
        //
        ...
        floatingActionButton: Padding(
          ...
          child: SB(
            ...
            child: FloatingActionButton(
              onPressed: () async {
                dynamic result =
                    await Get.toNamed(NavRoute.productBatchLabelAssessmentAdd);
                if (result == true) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        child: SimpleFormsTileViewCard(
                          ...
                          onDelete: onDelete,
                          viewRouter: NavRoute.productBatchLabelAssessmentView,
                          updateRouter:
                              NavRoute.productBatchLabelAssessmentUpdate,
                          // deleteRouter: NavRoute.productBatchLabelAssessmentDelete,
                          ...
                          textFieldFourLabel: "Production Batch No",
                          textFieldFourData:
                              recordList?[index].productionBatchNumber ?? "",
                        ),
                        onTap: () {
                          //open view
