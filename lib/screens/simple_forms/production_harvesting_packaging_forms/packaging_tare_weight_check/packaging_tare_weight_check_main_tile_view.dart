...
class PackagingTareWeightCheckMainTileView extends StatefulWidget {
  const PackagingTareWeightCheckMainTileView({super.key});

  @override
  State<PackagingTareWeightCheckMainTileView> createState() =>
      _PackagingTareWeightCheckMainTileViewState();
}

class _PackagingTareWeightCheckMainTileViewState
    extends State<PackagingTareWeightCheckMainTileView> {
  ...
  Future<bool> loadRecords() async {
    try {
      recordList = await PackagingTareWeightCheck().fetchAllRecords();
      debugPrint(recordList!.isEmpty
          ? "Empty List"
          : "${recordList?[0].packagingTareWeightCheckId}");
      return true;
    } catch (e) {
  ...
  Future<bool> deleteRecord(PackagingTareWeightCheckModel dataObject) async {
    try {
      int recordId =
          await PackagingTareWeightCheck().deleteRecord(model: dataObject);
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: SimpleFormsAppBar(
            title: "Packaging Tare Weight Check", isMainTile: true),
        //
        ...
        floatingActionButton: Padding(
          ...
          child: SB(
            ...
            child: FloatingActionButton(
              onPressed: () async {
                dynamic result =
                    await Get.toNamed(NavRoute.packagingTareWeightCheckAdd);
                if (result == true) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        child: SimpleFormsTileViewCard(
                          ...
                          onDelete: onDelete,
                          viewRouter: NavRoute.packagingTareWeightCheckView,
                          updateRouter: NavRoute.packagingTareWeightCheckUpdate,
                          ...
                          textFieldFourLabel: "Packaging Format",
                          textFieldFourData:
                              recordList?[index].packagingFormat ?? "",
                        ),
                        onTap: () {
                          //open view
