...
class DailyScaleCheckRecordMainTileView extends StatefulWidget {
  const DailyScaleCheckRecordMainTileView({super.key});

  @override
  State<DailyScaleCheckRecordMainTileView> createState() =>
      _DailyScaleCheckRecordMainTileViewState();
}

class _DailyScaleCheckRecordMainTileViewState
    extends State<DailyScaleCheckRecordMainTileView> {
  ...
  Future<bool> loadRecords() async {
    try {
      recordList = await DailyScaleCheck().fetchAllRecords();
      debugPrint(recordList!.isEmpty
          ? "Empty List"
          : "${recordList?[0].dailyScaleCheckId}");
      return true;
    } catch (e) {
  ...
  Future<bool> deleteRecord(DailyScaleCheckModel dataObject) async {
    try {
      int recordId = await DailyScaleCheck().deleteRecord(model: dataObject);
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: SimpleFormsAppBar(
            title: "Daily Scale Check Record", isMainTile: true),
        //
        ...
        floatingActionButton: Padding(
          ...
          child: SB(
            ...
            child: FloatingActionButton(
              onPressed: () async {
                dynamic result =
                    await Get.toNamed(NavRoute.dailyScaleCheckRecordAdd);
                if (result == true) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        child: SimpleFormsTileViewCard(
                          ...
                          onDelete: onDelete,
                          viewRouter: NavRoute.dailyScaleCheckRecordView,
                          updateRouter: NavRoute.dailyScaleCheckRecordUpdate,
