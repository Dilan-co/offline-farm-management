...
class PLUStickerRecordMainTileView extends StatefulWidget {
  const PLUStickerRecordMainTileView({super.key});

  @override
  State<PLUStickerRecordMainTileView> createState() =>
      _PLUStickerRecordMainTileViewState();
}

class _PLUStickerRecordMainTileViewState
    extends State<PLUStickerRecordMainTileView> {
  ...
  Future<bool> loadRecords() async {
    try {
      recordList = await PluStickerRecord().fetchAllRecords();
      debugPrint(recordList!.isEmpty
          ? "Empty List"
          : "${recordList?[0].pluStickerId}");
      return true;
    } catch (e) {
  ...
  Future<bool> deleteRecord(PluStickerRecordModel dataObject) async {
    try {
      int recordId = await PluStickerRecord().deleteRecord(model: dataObject);
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        appBar:
            SimpleFormsAppBar(title: "PLU Sticker Record", isMainTile: true),
        //
        ...
        floatingActionButton: Padding(
          ...
          child: SB(
            ...
            child: FloatingActionButton(
              onPressed: () async {
                dynamic result =
                    await Get.toNamed(NavRoute.pluStickerRecordAdd);
                if (result == true) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        child: SimpleFormsTileViewCard(
                          ...
                          onDelete: onDelete,
                          viewRouter: NavRoute.pluStickerRecordView,
                          updateRouter: NavRoute.pluStickerRecordUpdate,
