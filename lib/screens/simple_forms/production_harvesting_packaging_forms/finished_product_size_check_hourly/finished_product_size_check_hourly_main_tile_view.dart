...
class FinishedProductSizeCheckHourlyMainTileView extends StatefulWidget {
  const FinishedProductSizeCheckHourlyMainTileView({super.key});

  @override
  State<FinishedProductSizeCheckHourlyMainTileView> createState() =>
      _FinishedProductSizeCheckHourlyMainTileViewState();
}

class _FinishedProductSizeCheckHourlyMainTileViewState
    extends State<FinishedProductSizeCheckHourlyMainTileView> {
  final StateController stateController = Get.find();
  late Future<bool> loadingFuture;
  List<FinishedProductSizeCheckHourlyModel>? recordList;
  ...
  Future<bool> loadRecords() async {
    try {
      recordList = await FinishedProductSizeCheckHourly().fetchAllRecords();
      debugPrint(recordList!.isEmpty
          ? "Empty List"
          : "${recordList?[0].finishedProductSizeCheckHourlyId}");
      return true;
    } catch (e) {
  ...
  Future<bool> deleteRecord(
      FinishedProductSizeCheckHourlyModel dataObject) async {
    try {
      int recordId = await FinishedProductSizeCheckHourly()
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: SimpleFormsAppBar(
            title: "Finished Product Size Check (Hourly)", isMainTile: true),
        //
            ...
            child: FloatingActionButton(
              onPressed: () async {
                dynamic result = await Get.toNamed(
                    NavRoute.finishedProductSizeCheckHourlyAdd);
                if (result == true) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        child: SimpleFormsTileViewCard(
                          ...
                          onDelete: onDelete,
                          viewRouter:
                              NavRoute.finishedProductSizeCheckHourlyView,
                          updateRouter:
                              NavRoute.finishedProductSizeCheckHourlyUpdate,
