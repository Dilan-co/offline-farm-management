...
class InHouseLabelPrintingMainTileView extends StatefulWidget {
  const InHouseLabelPrintingMainTileView({super.key});

  @override
  State<InHouseLabelPrintingMainTileView> createState() =>
      _InHouseLabelPrintingMainTileViewState();
}

class _InHouseLabelPrintingMainTileViewState
    extends State<InHouseLabelPrintingMainTileView> {
  ...
  Future<bool> loadRecords() async {
    try {
      recordList = await InHouseLabelPrinting().fetchAllRecords();
      debugPrint(recordList!.isEmpty
          ? "Empty List"
          : "${recordList?[0].inHouseLabelPrintingId}");
      return true;
    } catch (e) {
  ...
  Future<bool> deleteRecord(InHouseLabelPrintingModel dataObject) async {
    try {
      int recordId =
          await InHouseLabelPrinting().deleteRecord(model: dataObject);
        ...
        floatingActionButton: Padding(
          ...
          child: SB(
            ...
            child: FloatingActionButton(
              onPressed: () async {
                dynamic result =
                    await Get.toNamed(NavRoute.inHouseLabelPrintingAdd);
                if (result == true) {
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        child: SimpleFormsTileViewCard(
                          ...
                          onDelete: onDelete,
                          viewRouter: NavRoute.inHouseLabelPrintingView,
                          updateRouter: NavRoute.inHouseLabelPrintingUpdate,
                          ...
                          textFieldFourLabel: "Pack Date",
                          textFieldFourData: recordList?[index].packDate ?? "",
                        ),
                        onTap: () {
                          //open view
