...
class FinishedProductWeightCheckHourlyAdd extends StatefulWidget {
  final bool isUpdate;
  final FinishedProductWeightCheckHourlyModel? data;
  const FinishedProductWeightCheckHourlyAdd({
    super.key,
    this.isUpdate = false,
    this.data,
  });
  ...
  @override
  State<FinishedProductWeightCheckHourlyAdd> createState() =>
      _FinishedProductWeightCheckHourlyAddState();
}

class _FinishedProductWeightCheckHourlyAddState
    extends State<FinishedProductWeightCheckHourlyAdd> {
  ...
  timeSlotVariableGenerator() {
    for (int index = 0;
        index < stateController.timeSlotsHourly.length;
        index++) {
      //grossWeight
      variableMap['grossWeight']?[stateController.timeSlotsHourly[index]] = "";
      //correctiveAction
      variableMap['correctiveAction']?[stateController.timeSlotsHourly[index]] =
          "";
      //comment
      variableMap['comment']?[stateController.timeSlotsHourly[index]] = "";
    }
    ...
    if (widget.data != null) {
      try {
        final hourlyWeightCheckMap =
            jsonDecode(widget.data!.hourlyWeightCheck ?? "");
  ...
  onChangedTimeSlotComment(String? output, int index) {
    setState(() {
      variableMap['comment']?[stateController.timeSlotsHourly[index]] = output;
    });
    debugPrint(output);
  }
  ...
  mapCreateHourlyWeightCheck({required Map<String, Map<String, dynamic>> map}) {
    try {
      for (int index = 0;
          index < stateController.timeSlotsHourly.length;
          index++) {
        map[stateController.timeSlotsHourly[index]] = {
          "id": stateController.timeSlotsHourly[index],
          "start_time": stateController.timeSlotsHourly[index].split('-').first,
          "end_time": stateController.timeSlotsHourly[index].split('-').last,
  ...
  Future<int> createRecord() async {
    try {
      ...
      await mapCreateHourlyWeightCheck(map: map);
      hourlyWeightCheck = jsonEncode(map);
      debugPrint("$map");
      debugPrint(hourlyWeightCheck);

      FinishedProductWeightCheckHourlyModel dataModel =
          FinishedProductWeightCheckHourlyModel(
              finishedProductWeightCheckHourlyId: null,
              client: client!,
              farm: farm!,
              crop: crop!,
              date: date!,
              hourlyWeightCheck: hourlyWeightCheck,
              netWeight: netWeight,
      ...
      FinishedProductWeightCheckHourlyModel dataModel =
          FinishedProductWeightCheckHourlyModel(
              finishedProductWeightCheckHourlyId:
                  widget.data!.finishedProductWeightCheckHourlyId,
              client: client!,
              farm: farm!,
              crop: crop!,
              date: date!,
  ...
  onSave() async {
    ...
    if (client != null && farm != null && crop != null && date != null) {
      ...
      if (widget.isUpdate) {
        Get.close(2);
        Get.to(() => const FinishedProductWeightCheckHourlyMainTileView());
      } else {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      appBar: SimpleFormsAppBar(
          title: "Finished Product Weight Check (Hourly)",
