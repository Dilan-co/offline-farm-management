...
class HarvestRiskAssessmentDataService {
  final StateController stateController = Get.find();

  String get formDataUrl =>
      '${stateController.baseUrl}/sf-harvest-risk-assessment-whp-observation';
  ...
  Future<bool> postData() async {
    ...
    List<HarvestRiskAssessmentModel> recordList =
        await HarvestRiskAssessment().fetchNotSyncedRecords();

    //Delete "delete_date" Expired Records
    HarvestRiskAssessment().deleteExpiredRecords();
        ...
        try {
          ...
          if (response.statusCode == 201 || response.statusCode == 200) {
            ...
            bool sig = await sendMultipartFormRequest(
              url: documentUrl,
              token: bearerToken,
              recordId: responseDecoded["harvest_risk_assessment_id"].toString(),
              modelName: 'SFHarvestRiskAssessmentWhpObservation',
              fieldName: 'signature',
              fileType: 'signature',
            ...
            debugPrint(
                'POST SIMPLE_FORM_harvest_risk_assessment_whp_observation Data Sync Failed: ${response.statusCode} - ${response.body}');
            return false;
          }
        } catch (e) {
          debugPrint(
              'POST SIMPLE_FORM_harvest_risk_assessment_whp_observation Error: $e');
          return false;
        }
      }
  ...
  addSyncAndExpireDate({required HarvestRiskAssessmentModel record}) async {
    ...
    //Update Record
    int recId = await HarvestRiskAssessment().updateRecord(model: record);
