...
class EquipmentMaintenanceLogDataService {
  final StateController stateController = Get.find();

  String get formDataUrl => '${stateController.baseUrl}/sf-equipment-maintenance-log';
  ...
  Future<bool> postData() async {
    ...
    List<EquipmentMaintenanceLogModel> recordList =
        await EquipmentMaintenanceLog().fetchNotSyncedRecords();

    //Delete "delete_date" Expired Records
    EquipmentMaintenanceLog().deleteExpiredRecords();
        ...
        try {
          ...
          if (response.statusCode == 201 || response.statusCode == 200) {
            ...
            bool file1 = await sendMultipartFormRequest(
              url: documentUrl,
              token: bearerToken,
              recordId: responseDecoded["equipment_maintenance_log_id"].toString(),
              modelName: 'SFEquipmentMaintenanceLog',
              fieldName: 'media',
              fileType: 'file',
            ...
            debugPrint(
                'POST SIMPLE_FORM_equipment_maintenance_log Data Sync Failed: ${response.statusCode} - ${response.body}');
            return false;
          }
        } catch (e) {
          debugPrint('POST SIMPLE_FORM_equipment_maintenance_log Error: $e');
          return false;
        }
      }
  ...
  addSyncAndExpireDate({required EquipmentMaintenanceLogModel record}) async {
    ...
    //Update Record
    int recId = await EquipmentMaintenanceLog().updateRecord(model: record);
