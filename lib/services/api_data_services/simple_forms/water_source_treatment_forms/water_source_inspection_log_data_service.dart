...
class WaterSourceInspectionLogDataService {
  final StateController stateController = Get.find();

  String get formDataUrl =>
      '${stateController.baseUrl}/sf-water-source-inspection-log';
  ...
  Future<bool> postData() async {
    ...
    List<WaterSourceInspectionLogModel> recordList =
        await WaterSourceInspectionLog().fetchNotSyncedRecords();

    //Delete "delete_date" Expired Records
    WaterSourceInspectionLog().deleteExpiredRecords();
        ...
        try {
          ...
          if (response.statusCode == 201 || response.statusCode == 200) {
            ...
            bool file1 = await sendMultipartFormRequest(
              url: documentUrl,
              token: bearerToken,
              recordId: responseDecoded["water_source_inspection_log_id"].toString(),
              modelName: 'SFWaterSourceInspectionLog',
              fieldName: 'media',
              fileType: 'file',
            ...
            debugPrint(
                'POST SIMPLE_FORM_water_source_inspection_log Data Sync Failed: ${response.statusCode} - ${response.body}');
            return false;
          }
        } catch (e) {
          debugPrint('POST SIMPLE_FORM_water_source_inspection_log Error: $e');
          return false;
        }
      }
  ...
  addSyncAndExpireDate({required WaterSourceInspectionLogModel record}) async {
    ...
    //Update Record
    int recId = await WaterSourceInspectionLog().updateRecord(model: record);
