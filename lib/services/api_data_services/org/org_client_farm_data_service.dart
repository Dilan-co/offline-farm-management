...
class OrgClientFarmDataService {
  final StateController stateController = Get.find();

  String get dataUrl => '${stateController.baseUrl}/farm';
  ...
  Future<bool> getData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<OrgClientFarmModel> dataList = List<OrgClientFarmModel>.from(
            responseDecoded.map((data) => OrgClientFarmModel.fromJson(data)));
        // Attempt batch insert/update
        try {
          if (dataList.isNotEmpty) {
            await OrgClientFarm().createRecordsBatch(models: dataList);
          }
        } catch (e) {
        ...
        debugPrint("GET ORG_client_farm Data Sync Failed");
        return false;
      }
    } catch (e) {
      debugPrint("GET ORG_client_farm Error : $e");
      return false;
    }
  }
}
